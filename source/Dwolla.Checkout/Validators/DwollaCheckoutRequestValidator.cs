...
namespace Dwolla.Checkout.Validators
{
    public class DwollaCheckoutRequestValidator : AbstractValidator<DwollaCheckoutRequest>
    {
        public DwollaCheckoutRequestValidator()
        {
            ...
            RuleFor(cr => cr.AllowFundingSources)
                .Equal(true)
                .When(cr => cr.AdditionalFundingSources != null)
                .WithMessage(
                    "The 'AllowFundingSources' should be 'true' when setting 'AdditionalFundingSources'. See off-site gateway documentation for more information.");
